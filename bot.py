...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("⚡ أرسل الصور وسيتم تحديث زر التحويل تلقائياً بأعلى سرعة!")

async def process_photos_instantly(user_id: int, chat_id: int, context: ContextTypes.DEFAULT_TYPE):
    # انتظر جزء بسيط من الثانية لجمع الصور التي تصل بنفس الميكروثانية معاً
    ...
    # زر التحويل المباشر مع عدد الصور
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton(f"✅ Done (تحويل {count} صورة)", callback_data="done_photos")],
        [InlineKeyboardButton("❌ إلغاء", callback_data="cancel_action")]
    ])

    # 1. مسح الرسالة القديمة فوراً لكي لا تبقى معلقة بالكروب/الشات فوق
    ...
    # 2. إرسال رسالة جديدة فورية في القاع (تحت آخر صورة وصلت الآن)
    try:
...
async def handle_photos(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # إلغاء المهمة السابقة وتحديث الرسالة فوراً بالأسفل
    if data.get("timer_task"):
        data["timer_task"].cancel()

    data["timer_task"] = asyncio.create_task(process_photos_instantly(user_id, chat_id, context))
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "done_photos":
        ...
        try:
            await query.edit_message_text(text="⚡ جاري التحويل بلمح البصر...")
        except Exception:
            pass

        try:
            # تنزيل الصور بالتوازي فوراً
