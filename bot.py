...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("⚡ البوت يعمل بأقصى سرعة صاروخية وبدون أي تأخير!")
...
async def handle_photos(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 1. مسح الرسالة القديمة فوراً من الأعلى
    ...
    # 2. إرسال الرسالة الجديدة فوراً بنفس اللحظة في أسفل الشات
    try:
...
# تنزيل فوري بالتوازي في الذاكرة
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "done_photos":
        ...
        try:
            await query.edit_message_text(text="⚡ جاري تحويل الصور...")
        except Exception:
            pass

        try:
            # تنزيل سريع ومباشر بكفاءة عالية
