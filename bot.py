...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("⚡ البوت يعمل بأعلى سرعة وبدون أخطاء الآن!")
...
# دالة تنزيل فائقة السرعة وآمنة من أخطاء الرابط
async def download_one_file(context, file_id):
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "done_photos":
        ...
        try:
            # تنزيل جميع الصور بالتوازي الصريح لتجنب أخطاء الشبكة والسرعة
            download_tasks = [download_one_file(context, fid) for fid in file_ids]
            photos_bytes = await asyncio.gather(*download_tasks)

            # التحويل المباشر لـ PDF في الذاكرة
            ...
            await context.bot.send_message(chat_id=query.message.chat_id, text="❌ حدث خطأ أثناء التحويل، حاول مرة أخرى.")
